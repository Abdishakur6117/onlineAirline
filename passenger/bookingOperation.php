...
function get_passenger($conn) {
    ...
    $passenger_id = $_SESSION['user_id']; // Use the same key set during login
...
function create_booking($conn) {
    $required = ['passenger_id', 'flight_id','seat_number','booking_date'];
    $data = [];
...
function update_booking($conn) {
    ...
    $stmt = $conn->prepare("
        SELECT booking_id FROM bookings 
        WHERE seat_number = ?  AND booking_id != ?
    ");
    $stmt->execute([
