        ...
        <div class="dashboard-wrapper">
            <div class="dashboard-ecommerce">
                <div class="container-fluid dashboard-content ">
                    <h2>Booking Form</h2>
                    <button type="button" class="btn btn-primary at-3" id="insertModal">Add Booking</button>
                    <br>
                    <br>
                ...
                <!--/   INsert Modal start -->
                <div class="modal fade" id="bookingModal" tabindex="-1" role="dialog" aria-labelledby="userModalLabel" aria-hidden="true">
                ...
                <!--/   INsert Modal end -->
                <!-- start Update Model  -->
                <div class="modal fade" id="edit_bookingModal" tabindex="-1" role="dialog" aria-labelledby="userModalLabel" aria-hidden="true">
                    <div class="modal-dialog modal-lg" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                            ...
                            <div class="modal-body">
                                <form id="edit_bookingForm" method="POST" action="">
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                <label for="username">Passenger Name </label>
                                                <input type="hidden" class="form-control" id="edit_id" name="edit_id">
                                                <select class="form-control" name="edit_passenger_id" id="edit_passenger_id">
                                                  <option value="">Select Passenger</option>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                <label for="username">Flight Number </label>
                                                <select class="form-control" name="edit_flight_id" id="edit_flight_id">
                                                  <option value="">Select Flight</option>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                <label for="username"> Seat Number </label>
                                                <input type="text" class="form-control" id="edit_seat_number" name="edit_seat_number">
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                <label for="password">Booking Date </label>
                                                <input type="datetime-local" class="form-control" id="edit_booking_date" name="edit_booking_date">
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="form-group">
    ...
    <script>
        $(document).ready(function() {
            ...
            function loadPassenger() {
                $.ajax({
                    url: 'bookingOperation.php?action=get_passenger',
                    method: 'GET',
                    dataType: 'json',
                    success: function(response) {
                        if(response.status === 'success' && response.data) {
                            const $select = $('#passenger_id, #edit_passenger_id');
            ...
            function loadFlight() {
                $.ajax({
                    url: 'bookingOperation.php?action=get_flight',
                    method: 'GET',
                    dataType: 'json',
                    success: function(response) {
                        if(response.status === 'success' && response.data) {
                            const $select = $('#flight_id, #edit_flight_id');
                            $select.empty().append('<option value="">Select flight</option>');
                            ...
                            showError('Failed to load flight');
                        }
                    },
                    error: function() {
                        showError('Network error loading flight');
                    }
                });
            }
            // Create user record
            $('#bookingForm').submit(function(e) {
                ...
                $.ajax({
                    type: 'POST',
                    url: 'bookingOperation.php?action=create_booking',
                    data: $(this).serialize(),
                    dataType: "json",
            ...
            $(document).on('click', '.editBtn', function() {
                const bookingData = {
                    id: $(this).data('id'),
                    passenger_id: $(this).data('passenger_id'),
                ...
                $('#edit_id').val(bookingData.id);
                $('#edit_passenger_id').val(bookingData.passenger_id);
                $('#edit_flight_id').val(bookingData.flight_id);
                $('#edit_seat_number').val(bookingData.seat_number);
                $('#edit_booking_date').val(bookingData.booking_date);
                $('#edit_status').val(bookingData.status);
                
                $('#edit_bookingModal').modal('show');
            });
            
            // Update user record
            $('#edit_bookingForm').submit(function(e) {
                e.preventDefault();
                const submitBtn = $(this).find('[type="submit"]');
                submitBtn.prop('disabled', true).html('<i class="fas fa-spinner fa-spin"></i> Updating...');
                const formData = {
                  edit_id: $('#edit_id').val(),
                  edit_passenger_id: $('#edit_passenger_id').val(),
                  edit_flight_id: $('#edit_flight_id').val(),
                  edit_seat_number: $('#edit_seat_number').val(),
                  edit_booking_date: $('#edit_booking_date').val(),
                  edit_status: $('#edit_status').val()
                };
                $.ajax({
                    url: 'bookingOperation.php?action=update_booking',
                    method: 'POST',
                    data: $(this).serialize(),
                    dataType: 'json',
                    success: function(response) {
                    ...
                    complete: function() {
                        submitBtn.prop('disabled', false).html('Update Booking');
                    }
                });
            });
            // Delete user record
            $(document).on('click', '.deleteBtn', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            type: 'POST',
                            url: 'bookingOperation.php?action=delete_booking',
                            data: { id: booking_id },
                            dataType: 'json',
                            success: function(res) {
                                if (res.status === 'success') {
            ...
            function displayData() {
                $.ajax({
                    url: 'bookingOperation.php?action=display_booking',
                    dataType: 'json',
                    success: function(response) {
                            ...
                            <tr>
                                ...
                                <td>
                                    <button class="btn btn-warning btn-sm editBtn" 
                                        ...
                                        data-flight_id="${row.flight_id}"
                                        data-seat_number="${row.seat_number}"
