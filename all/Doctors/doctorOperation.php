...
function loadData(){
  ...
  try {
    ...
    // $usersQuery = "SELECT u.user_id, u.fullName, d.consultation_fee 
    //                    FROM Users u 
...
//   if(!$doctor_id){
//     echo json_encode(["status" => "error", "message" => "doctor_id  is required."]);
//       return;
//   }
//   if(!$specialization){
...
//   }
//   //checks User_id
//   $user_idCheck = "SELECT * FROM Doctors WHERE  doctor_id  =:doctor_id  ";
//   $stm =$conn->prepare($user_idCheck);
//   $stm->bindParam(':doctor_id',$doctor_id);
//   $stm->execute();
//   $user_idExists = $stm->fetchColumn();
//   if($user_idExists > 0){
//     echo json_encode(["status" => "error", "message" => 'doctor_id already exists ']);
//     return;

//   }

//    // INSERT query with correct column names in database
//    $query = "INSERT INTO Doctors  (user_id ,specialization  ,availability,consultation_fee ) VALUES (:user_id,:specialization,:availability,:consultation_fee)";
