...
function createUser(){
  ...
  if($emailExists > 0){
   ...
   $query = "INSERT INTO Users  (fullName ,email  ,userName,password,phone ) VALUES (:username,:email,:userName,:password,:phone)";
   $stm= $conn->prepare($query);
   $stm->bindParam(':username', $fullName);
   $stm->bindParam(':email', $email);
...
function UpdateUser() {
  ...
  if (!empty($password)) {
      ...
      $query = 'UPDATE Users SET user_id = :user_id, fullName = :fullName, email = :email, username = :username, password = :password,phone=:phone,role=:role WHERE user_id = :user_id';
  } else {
      // If password is empty, update other fields without touching password
      $query = 'UPDATE Users SET user_id = :user_id, fullName = :fullName, email = :email, username = :username,phone = :phone,role = :role WHERE user_id = :user_id';
  }
  ...
  if (!empty($password)) {
      ...
      $stmt->execute([
          ':user_id' => $user_id,
          ':fullName' => $fullName,
          ':email' => $email,  // Store plain password
          ':username' => $username,
          ':password' => $password,
          ':phone' => $phone,
          ':role' => $role
